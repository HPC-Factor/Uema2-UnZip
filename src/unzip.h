/*===================================================================
   ...
   Copyright(c) 1993-1999 by CSD,inc. and shoda T. All rights reserved.
	...
	unzip.h
	WindowsCE's DLL version by uema2. All rights reserved ?
			version 0.01	2002/3/X
=====================================================================*/
#if !defined(UNZIPCE_H)
#define UNZIPCE_H

#define UNZIPCE_DLL_VERSION	93
...
typedef struct {
	DWORD	dwOriginalSize;		/* ÉtÉ@ÉCÉãÇÃÉTÉCÉYÅB		*/
	DWORD	dwCompressedSize;	/* à≥èkå„ÇÃÉTÉCÉYÅB		*/
	DWORD	dwCRC;			/* äiî[ÉtÉ@ÉCÉãÇÃÉ`ÉFÉbÉNÉTÉÄ/CRC */
	UINT	uFlag;			/* âìÄÇ‚ÉeÉXÉgÇÃèàóùåãâ 	*/
	UINT	uOSType;		/* Ç±ÇÃÉtÉ@ÉCÉãÇÃçÏê¨Ç…égÇÌÇÍÇΩÇnÇrÅB*/
	WORD	wRatio;			/* à≥èkó¶ÅiÉpÅ[É~Éã)		*/
	WORD	wDate;			/* äiî[ÉtÉ@ÉCÉãÇÃì˙ïtÅB		*/
	WORD	wTime;			/* äiî[ÉtÉ@ÉCÉãÇÃéûçèÅB		*/
	char	szFileName[FNAME_MAX32 + 1];/* ÉAÅ[ÉJÉCÉuÉtÉ@ÉCÉãñºÅ	*/
	char	dummy1[3];
	char	szAttribute[8];		/* äiî[ÉtÉ@ÉCÉãÇÃëÆê´ÅB		*/
	char	szMode[8];		/* äiî[ÉtÉ@ÉCÉãÇÃäiî[ÉÇÅ[ÉhÅB	*/
} INDIVIDUALINFO, FAR *LPINDIVIDUALINFO;

typedef struct {
	DWORD	dwFileSize;			/* äiî[ÉtÉ@ÉCÉãÇÃÉTÉCÉY	*/
	DWORD	dwWriteSize;			/* âìÄÇµÇƒèëÇ´çûÇÒÇæÉTÉCÉY */
	char	szSourceFileName[FNAME_MAX32 + 1]; /* èàóùÇçsÇ§äiî[ÉtÉ@ÉCÉãñº */
	char	dummy1[3];
	char	szDestFileName[FNAME_MAX32 + 1];	/* é¿ç€Ç…èëÇ´çûÇ‹ÇÍÇÈÉpÉXñº */
	char	dummy[3];
...
typedef struct {
	...
	DWORD dwCompressedSize;	/* à≥èkå„ÇÃÉTÉCÉYÅB*/
	DWORD dwCRC;			/* äiî[ÉtÉ@ÉCÉãÇÃÉ`ÉFÉbÉNÉTÉÄ/CRC */
	UINT  uOSType;			/* Ç±ÇÃÉtÉ@ÉCÉãÇÃçÏê¨Ç…égÇÌÇÍÇΩÇnÇr */
	WORD  wRatio;			/* à≥èkó¶ÅiÉpÅ[É~Éã) */
	WORD  wDate;			/* äiî[ÉtÉ@ÉCÉãÇÃì˙ïtÅB*/
	WORD  wTime;			/* äiî[ÉtÉ@ÉCÉãÇÃéûçèÅB*/
	char  szAttribute[8];	/* äiî[ÉtÉ@ÉCÉãÇÃëÆê´ÅB*/
...
/* LHA.DLL Compatible API (LHA.DLL V1.1 Ç∆å›ä∑ê´Ççló∂ÇµÇΩ API åQÇ≈Ç∑Åj
 */
...
int _export UnZip(const HWND hWnd, LPCTSTR szCmdLine, LPTSTR szOutput,const DWORD wSize);
WORD _export UnZipGetVersion(VOID);

// comment-outed by uema2. 
// It will be implemented step by step...

BOOL _export UnZipGetRunning(VOID);
/*
...
/* True Compatible API (ÅwìùçáÉAÅ[ÉJÉCÉoÇ`ÇoÇhÅxì∆é©ÇÃÅAå›ä∑ê´ÇÃçÇÇ¢ API åQÇ≈Ç∑Åj
	êVãKÇ…égópÇ≥ÇÍÇÈèÍçáÇÕÅAèoóàÇÈÇæÇØÇ±ÇøÇÁÇÇ®égÇ¢Ç≠ÇæÇ≥Ç¢ÅB
	íçÅFåªç›çÙíËíÜÇ≈ÅAãÔëÃìIÇ…åàíËÇµÇΩÇ‡ÇÃÇÕâ∫ãLÇÃÇ›Ç≈Ç∑ÅB
	    Ç≤à”å©Å^Ç≤óvñ]ìôÇÕ NIFTY-Serve  FWINDC Mes #3 Ç÷ÅI
 */
/*
...
#define	CHECKARCHIVE_RAPID	0 /* ä»à’å^ÅiçÇë¨Åj */
#define	CHECKARCHIVE_BASIC	1 /* ïWèÄå^ÅiÉwÉbÉ_Å[ÇÃÇ›Åj */
#define	CHECKARCHIVE_FULLCRC	2 /* äÆëSå^ÅiÇbÇqÇbìôÇÃÉ`ÉFÉbÉNÇä‹ÇﬁÅj */
#endif
/*
BOOL WINAPI _export UnZipConfigDialog(const HWND hwnd,LPSTR szOption,const int iMode);
*/
#if !defined(UNPACK_CONFIG_MODE)
#define	UNPACK_CONFIG_MODE	1 /* âìÄÅiïúå≥ÅjånÇÃÉRÉ}ÉìÉh */
#define	PACK_CONFIG_MODE	2 /* à≥èkÅiçÏê¨ÅjånÇÃÉRÉ}ÉìÉh */
#endif
/*
...
/* äiî[Ç≥ÇÍÇƒÇ¢ÇÈÉtÉ@ÉCÉãÇÃèÓïÒÇÃéÊìæÅB
 */
...
#define	ARCEXTRACT_BEGIN	0	/* äYìñÉtÉ@ÉCÉãÇÃèàóùÇÃäJén */
#define	ARCEXTRACT_INPROCESS	1	/* äYìñÉtÉ@ÉCÉãÇÃìWäJíÜ */
#define	ARCEXTRACT_END		2	/* èàóùèIóπÅAä÷òAÉÅÉÇÉäÇäJï˙ */
/* à»â∫ÇÕ UNZIP Ç≈ÇÕégópÇµÇƒÇ¢Ç»Ç¢ */
#define ARCEXTRACT_OPEN			3	/* äYìñèëå…ÇÃèàóùÇÃäJén */
#define ARCEXTRACT_COPY			4	/* ÉèÅ[ÉNÉtÉ@ÉCÉãÇÃèëÇ´ñﬂÇµ */
...
/* DMZIP32.DLL Ç∆ÇÃå›ä∑ÇÃÇΩÇﬂÇ…Ç†ÇËÇ‹Ç∑ÅB
   UNZIP32.DLL ÇÃÉoÅ[ÉWÉáÉìÇï‘ÇµÇ‹Ç∑Ç™ÅAUNZIP32.DLL Çé¶Ç∑ÇΩÇﬂÇ…ÅA0x2000 Ç™
   â¡éZÇ≥ÇÍÇƒÇ¢Ç‹Ç∑ÅiUnZipGetVersion() + 0x2000ÅjÅB
 */
/*
...
#define M_RECOVERY_ON			0x08000000L /* îjëπÉwÉbÉ_ÇÃì«Ç›îÚÇŒÇµ */

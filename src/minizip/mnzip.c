...
//char* backup_CentralHeader     = NULL;
//char* backup_CentralEndHeader0 = NULL;
//char* backup_CentralEndHeader1 = NULL;
struct HeaderBackup centralHeader={0};
struct HeaderBackup centralEndHeader0={0};
struct HeaderBackup centralEndHeader1={0};

long backupHeader(FILE* fp);
int  checkCentralHeaderFileName(const char* fname);
long findPK56(FILE* fp, int mode);
long findPK12(FILE* fp, short num, long pk56size);
void restoreHeader(FILE* fp);
void setPK56(FILE* fp);
...
#ifdef _WIN32_WCE
# define VERSIONMADEBY   (0x15) /* platform depedent(WinCE) */
#else
...
//	ziinit.filezip = fopen(pathname,(append == 0) ? "wb" : "ab");
	ziinit.filezip = fopen(pathname,(append == 0) ? "wb" : "rb+");
	...
	// by uema2.
 	if( append==1 )
	{
		long size = backupHeader(ziinit.filezip);
		fseek(ziinit.filezip, -size, SEEK_END);
	}
	///////////
    ...
    int level;
{
	...
	if( base_directory != NULL ){
		pFileName = filename;
		pBaseDir  = base_directory;
		while( *pFileName != '\0' && *pBaseDir != '\0' ){
			if( *pFileName != *pBaseDir )
				break;
			pFileName = CharNextA(pFileName);
			pBaseDir  = CharNextA(pBaseDir);
		}
	}else{
		pFileName = filename;
	}
	size_filename = strlen( pFileName );
//    size_filename = strlen(filename);

	if( !checkCentralHeaderFileName(filename) ) 
		return ZIP_PARAMERROR;
    ...
    for (i=0;i<size_filename;i++)
        *(zi->ci.central_header+SIZECENTRALHEADER+i) = *(pFileName+i);
//        *(zi->ci.central_header+SIZECENTRALHEADER+i) = *(filename+i);
    ...
    for (i=0;i<size_comment;i++)
        *(zi->ci.central_header+SIZECENTRALHEADER+size_filename+
              size_extrafield_global+i) = *(pFileName+i);
...
//        if (fwrite(filename,(uInt)size_filename,1,zi->filezip)!=1)
        if (fwrite(pFileName,(uInt)size_filename,1,zi->filezip)!=1)
                err = ZIP_ERRNO;
...
// 以下、uema2.
/////////////////////////////////////////////////////////
extern int ZEXPORT zipClose2(zipFile file, const char* global_comment,
...
long backupHeader(FILE* fp)
{
	...
	// PK12 がいくつあるのかを取得してbackup
	// (short pk56[4])
	sp = (short*)centralEndHeader0.p;
	n12 = findPK12( fp, sp[4], n56 );
...
long findPK56(FILE* fp, int mode)
{
	...
	// Heade を読んで backup
	if( mode==0 ) bk = &centralEndHeader0;
	else          bk = &centralEndHeader1;

	bk->p = (char*)LocalAlloc( LPTR, size );
	bk->size = size;

	fseek(fp, -size, SEEK_END);
	fread( bk->p, size, 1, fp );
...
long findPK12(FILE* fp, short num, long pk56size)
{
	...
	centralHeader.p = 
		(char*)LocalAlloc( LPTR, size-pk56size );
	centralHeader.size = size-pk56size;

	fseek( fp, -size, SEEK_END );
	fread( centralHeader.p, size-pk56size, 1, fp );
...
void restoreHeader(FILE* fp)
{
	...
	// PK12 をくっつけ
//	setPK12();
	fseek(fp, -n56, SEEK_END);
	fwrite( centralHeader.p, centralHeader.size, 1, fp);

	// PK56 をくっつけ
	setPK56(fp);

	// すべてFree
	LocalFree( centralHeader.p );
	LocalFree( centralEndHeader0.p );
	LocalFree( centralEndHeader1.p );
}

//---------------------------------------------------------
void setPK56(FILE* fp)
{
	short *sp0 = (short*)centralEndHeader0.p;
	short *sp1 = (short*)centralEndHeader1.p;
	int   *ip0 = (int*)centralEndHeader0.p;
	int   *ip1 = (int*)centralEndHeader1.p;

	sp1[4] += sp0[4], sp1[5] += sp0[5];
	ip1[3] += ip0[3];

	fwrite( centralEndHeader1.p, 
		centralEndHeader1.size, 1, fp );
}

//---------------------------------------------------------
int checkCentralHeaderFileName(const char* fname)
{
	// centralHeader 内に、同じファイル名が
	// あったら false。
	char* p = centralHeader.p;
	short* sp = (short*)centralEndHeader0.p;
	int num, i, offset;
	...
	{
		sp = (short*)p;
		if( 0==strnicmp( fname, &p[46], sp[14] ) )
		{
			printfToBuffer( "same filename exists. delete first. ->" );
			printfToBuffer( fname );
			printfToBuffer( "\n" );
			return 0;
		}
		offset = 46 + sp[14]+sp[15]+sp[16];
		p += offset;
	}
	return 1;
}
